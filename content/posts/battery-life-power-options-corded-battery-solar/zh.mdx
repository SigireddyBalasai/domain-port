...
<Badge variant="outline">物理优先</Badge> **能量守恒: 输入 = 输出 + 损耗。**
营销改变不了这点。按*最差*情况定尺寸
(最短白天、最低温、最高活动)，而非最好情况。
...
**营销 vs 现实:** "6 个月续航" = 每天 10 次运动、10
秒片段、70°F、无实时预览。**真实世界:** 每天 20–40 次事件 + 5 次实时预览 =
**2–6 周**。永远按 3–5 倍降额。
...
<strong>IEEE 802.3af (PoE):</strong> PSE 端 15.4W → PD 端 (摄像头)
12.95W。驱动多数固定子弹/球机。
<strong>IEEE 802.3at (PoE+):</strong> PSE 端 30W → PD 端 25.5W。驱动
PTZ、加热器、IR 补光灯。
<strong>IEEE 802.3bt (PoE++):</strong> 60W (Type 3) / 90W (Type 4) PSE 端 → PD
端 51W / 71W。驱动高速球机、多传感器、雨刷/加热器。

<strong>线材:</strong> Cat5e 起 (Cat6/6a 用于 PoE++)。单段最长 100 米 (328
英尺)。
<strong>拓扑:</strong> 摄像头 → Cat5e/6 → PoE 交换机 (或带 PoE 口的 NVR) → UPS →
电网。
<strong>电压:</strong> 线对上 44–57V DC (Mode A: 数据线对 / Mode B:
备用线对)。摄像头内部 DC-DC 转 12V/5V/3.3V。
...
<strong>原则:</strong> UPS 必须覆盖
<strong>所有 PoE 交换机端口 + NVR + 路由器</strong> 以达目标续航。
...
| <strong>总计 (12 摄像头系统)</strong> | <strong>~170W</strong> | <strong>680 Wh</strong> | <strong>2,040 Wh</strong> | <strong>4,080 Wh</strong> |

<strong>UPS 推荐:</strong>

<ul>
  <li>
    <strong>&lt;4 小时:</strong> CyberPower CP1500PFCLCD (1,500 VA / 1,050 Wh) —
    $200
  </li>
  <li>
    <strong>8–12 小时:</strong> APC SMT1500RM2UC + 外置电池包 — $600+
  </li>
  <li>
    <strong>24+ 小时:</strong> 48V LiFePO₄ 机架电池 (5–10 kWh) + Victron
    逆变器/充电器 — $2,000+
  </li>
</ul>

<strong>专业提示:</strong> PoE 交换机 + NVR + 路由器放 <strong>同一 UPS</strong>
。摄像头端 UPS (单摄像头) 存在但同续航成本 5 倍。
...
**化学成分:** 绝大多数消费级电池摄像头用 **Li-ion (NMC/LCO), 3.6–3.7V 标称,
4.2V 最大**。不是 LiFePO₄。这对低温很重要。
...
<ol>
  <li>
    <strong>Tx 功率主导:</strong> Wi-Fi 射频 +17 dBm = 300–500 mA @ 3.7V。10
    秒片段 =
  </li>
</ol>
<ol>
  <li>
    <strong>IR LED:</strong> 850 nm IR 100 英尺 = 1–2W 持续 30 秒/片段。30 片段
    = 0.25–0.5 Wh = <strong>70–140 mAh @ 3.7V</strong>。
  </li>
  <li>
    <strong>PIR 唤醒 + DSP:</strong> 每事件 50–100 mA 持续 2–5 秒。单独可忽略,
    累积可观。
  </li>
  <li>
    <strong>低温:</strong> Li-ion <strong>32°F (0°C) 内阻翻倍</strong>。Tx
    负载下电压跌落 → BMS 在 3.0V 切断 → "没电" 实为 40% SoC。
    <strong>14°F (-10°C) 容量 ≈ 70°F 的 50%。</strong>
  </li>
  <li>
    <strong>自放电:</strong> 2–5%/月。相对主动耗电可忽略。
  </li>
  <li>
    <strong>实时预览:</strong> 5 分钟实时 = 30+ 片段能量。
    <strong>避免每日查看实况。</strong>
  </li>
</ol>
  ...
  <AccordionItem value="charging">
    ...
    <AccordionContent>
      ...
      <strong>别等 0%。</strong> Li-ion 怕深放电。20–30% 时充电。 <strong>太阳能板定型:</strong> 面板 (W)
      ≥ 摄像头平均功耗 (W) × 3 (冬天/阴天) ÷ 峰值日照小时 (最差月)。 - 例: Argus
        4 Pro 平均 1.5W → 需 4.5W。最差月 (12 月, 5 区) = 1.5 峰值小时 → <strong>最小 3W
      面板, 推荐 6W</strong>。 <strong>USB-C PD 触发线:</strong> Reolink/Argus/Eufy 接受
        5V/9V/12V/15V/20V 经 PD 协商。用 12V→USB-C PD 触发线从 12V
      房车/家用电池组直充 (90% 效率 vs 12V→120V 逆变器→5V 适配器 60%)。
      <strong>双电池轮换:</strong> 买备用包。满电换没电。零停机。仅限用户可拆卸电池包
      (Reolink, Blink, 部分 Ring)。
...
**太阳能是电池充电器, 非电源。** 为**自主天数** (无日照天数)
定**电池**容量。为**1 个好天**充满该电池定**面板**瓦数。
...
<strong>12 月冬至 (5 区, 42°N):</strong>

<ul>
  <li>
    峰值日照小时: <strong>1.0–1.5</strong> (6 月 5.5)
  </li>
  <li>
    30° 倾角面板输出: <strong>STC 额定值的 15–20%</strong>
  </li>
  <li>
    积雪覆盖: <strong>0% 输出</strong> 直到清理 (自热面板存在: 5–10W 寄生功耗)
  </li>
  <li>
    电池 14°F: <strong>Li-ion = 50% 容量; LiFePO₄ = 80% 容量</strong>
  </li>
</ul>
...
<ol>
  <li>
    <strong>面板按夏季计算 3–4 倍</strong> (60W → 180–240W 阵列)
  </li>
  <li>
    <strong>LiFePO₄ 电池</strong> (非 Li-ion) — -4°F 仍可随 BMS 加热器充电
  </li>
  <li>
    <strong>降低摄像头占空比:</strong> 仅运动、降分辨率、缩短片段、关 IR
    (用环境光)
  </li>
  <li>
    <strong>备用充电:</strong> 12V→USB-C PD 触发线从车/发电机月充一次
  </li>
  <li>
    <strong>接受停机:</strong> 设计 90% 正常运行, 非 100%。每年 3–5 天黑屏正常。
  </li>
</ol>
...
**隐性成本:** 跑腿。电池摄像头凌晨 3 点死 → 开车 30 分钟换 = $50/次。PoE + UPS
= 0 次因电源跑腿。按 $50 × 预期年故障次数计入。
